...
*/
/*
  "alg_mfe_macrostate" is the same as "alg_mfe", but with a four-tuple as answer
  type.
*/
algebra alg_mfe implements sig_foldrna(alphabet = char, answer = answer_macrostate_mfe) {
  ...
  answer_macrostate_mfe mladldr(Subsequence lb,Subsequence dl,answer_macrostate_mfe e,Subsequence dr,Subsequence rb) {
    ...
    res.energy = ml_energy() + ul_energy() + e.energy + min(dli_energy(closingStem, closingStem), dl_energy(firstStem, firstStem)) + dri_energy(closingStem, closingStem) + termau_energy(closingStem, closingStem);
    return res;
  }
