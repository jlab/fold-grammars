...
algebra alg_ali_outside_mfe_overdangle extends alg_ali_outside_mfe {
	...
	mfecovar outer_drem(Subsequence locr, mfecovar x, Subsequence locl) {
		mfecovar res = x;
		res.mfe = x.mfe + ((termau_energy(shiftIndex(locl), shiftLeftIndex(locr)) + ext_mismatch_energy(shiftIndex(locl), shiftLeftIndex(locr))) / float(rows(locl)));
		return res;
	}
...
algebra alg_ali_outside_pfunc_overdangle extends alg_ali_outside_pfunc {
	...
	double outer_drem(Subsequence locr, double x, Subsequence locl) {
		return x * mk_pf((termau_energy(shiftIndex(locl), shiftLeftIndex(locr)) + ext_mismatch_energy(shiftIndex(locl), shiftLeftIndex(locr))) / float(rows(locr)));
	}
