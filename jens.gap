...
algebra shape5 implements jensAlgebra(alphabet = char, comp = shape_t) {
...
algebra mfe implements jensAlgebra(alphabet = char, comp = int) {
  ...
  int dlr(Subsequence lb, int e, Subsequence rb) {
    return e + dl_energy(lb, rb) + dr_energy(lb, rb) + termaupenalty(lb, rb);
  }
  ...
  int ml(Subsequence bl, Subsequence f1, int x, Subsequence f2, Subsequence br) {
    return 380 + x + termaupenalty(f1, f2) + sr_energy(bl, br) + dli_energy(f1, f2) + dri_energy(f1, f2);
  }
...
algebra p_func implements jensAlgebra(alphabet = char, comp = double) {
  ...
  double ml(Subsequence bl, Subsequence f1, double x, Subsequence f2, Subsequence br) {
    return scale(4) * x * mk_pf(380 + termaupenalty(f1, f2) + sr_energy(bl, br) + dli_energy(f1, f2) + dri_energy(f1, f2));
  }
...
instance count = jensDangle (count);
instance enum = jensDangle (enum);

instance shape5pfx = jensDangle ((shape5 * p_func) suchthat p_func_filter);
instance shape4pfx = jensDangle ((shape4 * p_func) suchthat p_func_filter);
instance shape3pfx = jensDangle ((shape3 * p_func) suchthat p_func_filter);
