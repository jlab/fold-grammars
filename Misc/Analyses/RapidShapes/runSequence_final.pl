...
my $resultDir = '/vol/fold-grammars/src/Misc/Analyses/RapidShapes/ResultsFinal_random/'.$header.'/';
...
my $shapeCount = 1;
foreach my $dir ('/vol/fold-grammars/src/Misc/Analyses/RapidShapes/ResultsSample/OUT','/vol/fold-grammars/src/Misc/Analyses/RapidShapes/ResultsClean_random/OUT') {
	my $storedResultFile = Utils::execute("grep \"$header\" $dir/* | cut -d \":\" -f 1"); chomp $storedResultFile;
		...
		foreach my $header (keys(%{$tmp})) {
			...
			if (exists $tmp->{$header}->{subopt}) {
				foreach my $deviation (keys(%{$tmp->{$header}->{subopt}})) {
					print "run old RNAshapes -c $deviation: ".$tmp->{$header}->{subopt}->{$deviation}->{runtime}." sec., ".$tmp->{$header}->{subopt}->{$deviation}->{memory}." KB RSS. Found 0000 shape classes.\n";
				}
			}
			foreach my $shape (keys (%{$tmp->{$header}->{shapes}})) {
				if (not exists $analysedshapes{$shape}) {
					...
					print "TMP\t".$shape."\t".$tmp->{$header}->{shapes}->{$shape}->{energy}."\t".$tmp->{$header}->{shapes}->{$shape}->{pfuncValue}/$pfall."\t".$tmp->{$header}->{shapes}->{$shape}->{pfuncValue}."\t".$tmp->{$header}->{shapes}->{$shape}->{runtime}."\t".$tmp->{$header}->{shapes}->{$shape}->{memory}."\t".($shapeCount++)."\t".$exploredSearchspace."\n";
					if (exists $tmp->{$header}->{sample}) {				
						foreach my $samplesize (keys(%{$tmp->{$header}->{sample}})) {
...
my $noNewIteration = 0;
my $iteration = 1;
while(0 && ($exploredSearchspace < $minSearchSpace)) {
