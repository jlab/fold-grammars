...
my $WorkDir = Utils::absFilename($jobName.'/'.$grammar.'_mea/');

my $numberSeqs = Utils::execute(Settings::getBinary('cat')." $file | ".Settings::getBinary('grep').' "^>" -c'); 
chomp $numberSeqs;
#~ $numberSeqs = 10;
...
if (not -d $WorkDir) {
	...
	open (ARR, "> $WorkDir/array.sh") || die;
		...
		print ARR ''.Settings::getBinary('perl').' '.$Settings::rootDir.'/Misc/Analyses/Pareto/computeMEA.pl "'.$grammar.'" "$header'."\t".'$sequence'."\t".'$structure'."\t".'$reactivities"'."\n";
		print ARR ''."\n";
		print ARR '#'.$qsub."\n";
	close (ARR);
