	...
	while (my $line = <IN>) {
		if ($line =~ m/^(\S+)_main.cc :/) {
			...
			if ($mode == 2) {
				...
				$content .= "\t".$sedBinary.' -i \'s|gapc::class_name obj;|gapc::class_name obj;\n  outside_gapc::class_name outside_obj;\n  gapc::Opts outside_opts;\n  try {\n    optind = 1;\n    outside_opts.parse(argc, argv);\n    char *j = new char[gapc::Opts::getOpts()->inputs.front().second*2+1];\n    *j = 0;\n    std::strcat(j, gapc::Opts::getOpts()->inputs.front().first);\n    std::strcat(j, "+");\n    std::strcat(j, gapc::Opts::getOpts()->inputs.front().first);\n    outside_opts.inputs.clear();\n    std::pair<const char*, unsigned int> doubleinput = std::make_pair(j,gapc::Opts::getOpts()->inputs.front().second*2+1);\n    outside_opts.inputs.push_back(doubleinput);\n  } catch (std::exception \&e) {\n      std::cerr << "Exception: " << e.what() << '."'\\''\\\\n'\\''".';\n      std::exit(1);\n  }\n|\' '.$1.'_main.cc'."\n";
