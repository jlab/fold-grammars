...
if (defined $comment && $comment !~ m/^\s*$/) {
	foreach my $series (@SERIES) {
		qx(export DEBFULLNAME="$USER <$EMAIL>"; cd $pathContainingDebian && debchange --newversion $newversion-0ubuntu1 $comment --package $package --distribution $series);
	}
	...
	mkdir $packageDir;
	qx(hg clone ../ $packageDir/$package);
	...
	print Dumper qx(dput ppa:bibi-help/bibitools $TMPDIR/${package}_$newversion-0ubuntu1_source.changes);
	#~ qx(rm -rf $TMPDIR);
} else {
