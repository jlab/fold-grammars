grammar gra_locomotif_overdangle uses sig_foldrna(axiom = struct) {
  ...
  dangle    = drem(LOC, sr(BASE, closed, BASE) with basepairing, LOC) # h;
  ...
  leftB     = bl(BASE, REGION with maxsize(30), sr(BASE, closed, BASE) with basepairing,                          BASE) # h;
  rightB    = br(BASE,                          sr(BASE, closed, BASE) with basepairing, REGION with maxsize(30), BASE) # h;
