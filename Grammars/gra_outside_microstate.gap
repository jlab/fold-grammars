...
//in case of no lonely base pairs: (nt_weak(i,j) * nt_outer_strong(j,n+i+1) + nt_strong(i,j) * nt_outer_weak(j,n+i+1)) / obj.nt_struct(0,n)
  ...
  outer_weak = outer_drem(LOC,  sep(struct, BASE with containsBase(N_BASE), struct), LOC )
			 | outer_edl (LOC,  sep(struct, BASE with containsBase(N_BASE), struct), BASE)
			 | outer_edr (BASE, sep(struct, BASE with containsBase(N_BASE), struct), LOC )
			 | outer_edlr(BASE, sep(struct, BASE with containsBase(N_BASE), struct), BASE)
  ...
  outer_mlfinal = outer_ml   (BASE,       incl(outer_dangle),       BASE) with basepair
                | outer_mldl (BASE,       incl(outer_dangle), BASE, BASE) with basepair
				| outer_mldr (BASE, BASE, incl(outer_dangle),       BASE) with basepair
				| outer_mldlr(BASE, BASE, incl(outer_dangle), BASE, BASE) with basepair
                # h;				 
