...
inline void empty(rules &e) { e.empty_ = true; }

inline bool isEmpty(const rules &e) { return e.empty_; }
...
inline void setShape(rules &me, Rope s) { me.setShape(s); }
inline void setShape(rules &me, const char *s) { me.setShape(Rope(s)); }
// In level 1 it might happen that two subshapes must be concatenated that both
// have unpaired bases at their tail and head, e.g. []_ and _[]. In such a case,
// the concatenation is not simply ++ ([]_ + _[] != []__[]), but must recognize
// the double unpaired stretch and fuse them into one, such that []_ + _[] =
// []_[]
...
inline void appendShape(rules &me, const char *y) { appendShape(me, Rope(y)); }
...
inline rules operator+(const rules &x, const rules &y) {
  ...
  for (nt = y.productions.begin(); nt != y.productions.end(); nt++) {
    for (rhs = nt->second.begin(); rhs != nt->second.end(); rhs++) {
      res.productions[nt->first].insert(
          std::pair<Rope, bool>(rhs->first, true));
    }
  }
  return res;
}
inline Rope toRope(const rules &me) { return me.toRope(); }

// for choice function: combines rules of several parses, which is necessary for
// the ambiguous grammar for shape level 1 and macrostate
inline rules merge(std::pair<List<rules, unsigned char>::iterator,
...
struct alignment {
  ...
  alignment() { empty_ = false; }

  alignment(int i) { empty_ = alignmentRows.size() == 0; }

  void empty() { empty_ = true; }
  bool isEmpty() const { return empty_; }
};
inline bool isEmpty(const alignment &a) { return a.empty_; }
inline bool is_not_empty(const alignment &a) { return !a.empty_; }
inline void empty(alignment &e) { e.empty_ = true; }
inline int rows(const alignment &a) { return a.alignmentRows.size(); }
...
inline void append(alignment &ali1, const alignment &ali2) {
  ...
  if ((static_cast<int>(ali2.alignmentRows.size())) > numberRows)
    numberRows = ali2.alignmentRows.size();
...
inline void append(alignment &ali1,
                   const Basic_Subsequence<M_Char, pos_type> &mchar) {
