...
inline bool unpaired(const Basic_Sequence<alphabet, pos_type> &seq, T i, T j) {
  for (T k = i; k < j; ++k) {
    if (seq[k] == SEPARATOR_BASE) return false;
  }
  return true;
}

// this function takes a REGION parse and returns a string composed of as many
// '.' as there are regular bases and a '+' char at the position of the
// SEPARATOR_BASE.
template <typename alphabet, typename pos_type>
inline void appendSeperatorRegion(
    String &res, const Basic_Subsequence<alphabet, pos_type> &seq) {
...
template <typename alphabet, typename pos_type>
inline Rope seperatorRegion_to_shape(
    const Basic_Subsequence<alphabet, pos_type> &seq, int shapelevel) {
