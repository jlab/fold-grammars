 ...
 public:
  ...
  void setStructure(std::pair<const char*, unsigned int> structure) {
    ...
    for (ntt = types.begin(); ntt != types.end(); ntt++) {
      stacks.insert(std::pair<char, std::vector<unsigned int> >(
          ntt->first, std::vector<unsigned int>()));
    }
    for (unsigned int i = 0; i < structure.second; i++) {
      ...
      if (getOpenType(structure.first[i]) !=
          '-') {  // position is a closing base
        if (stacks.at(getOpenType(structure.first[i])).size() < 1) {
          std::cerr << "Your structure is no valid dot bracket string: too few "
                       "opening brackets of type '"
                    << getOpenType(structure.first[i]) << "'." << std::endl;
          exit(1);
        }
        basepairs.insert(std::pair<unsigned int, unsigned int>(
            stacks.at(getOpenType(structure.first[i])).back(), i));
        stacks.at(getOpenType(structure.first[i])).pop_back();
      }
    }
    // check for valid structure. Invalid if one of the stacks contains an
    // element --> to many opening partners
    for (ntt = types.begin(); ntt != types.end(); ntt++) {
      if (stacks.at(ntt->first).size() > 0) {
        std::cerr << "Your structure is no valid dot bracket string: too many "
                     "opening brackets of type '"
                  << ntt->first << "'." << std::endl;
        exit(1);
      }
  ...
  bool isOpen(unsigned int i) { return basepairs.count(i) > 0; }
  ...
  unsigned int closingPartner(unsigned int i) { return basepairs.at(i); }
  void debugPrint() {
    std::map<unsigned int, unsigned int>::const_iterator nt;
    for (nt = basepairs.begin(); nt != basepairs.end(); nt++) {
      std::cout << "base-pair between " << nt->first << " and " << nt->second
                << std::endl;
    }
  }
...
inline bool basepair(const Basic_Sequence<alphabet, pos_type>& seq, T i, T j) {
  return (Pairs::getGivenPairs()->isOpen(i)) &&
         (Pairs::getGivenPairs()->closingPartner(i) == j - 1);
}
...
inline bool regionpair(int i, int j, int len) {
  int k;
  for (k = 0; k < len; k++) {
    if (!((Pairs::getGivenPairs()->isOpen(i + k)) &&
          ((static_cast<int>(Pairs::getGivenPairs()->closingPartner(i + k))) ==
           j - k - 1))) {  // opening position must be partner of closing
                           // position. Indices run in opposite directions!
      return false;
    }
  }
  return true;
