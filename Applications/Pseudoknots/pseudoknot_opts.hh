...
class Opts {
  ...
  public:
    ...
    Opts()
      ...
      energydeviation_absolute(std::numeric_limits<float>::infinity()),
      maximalPseudoknotSize(std::numeric_limits<int>::max()),
      minimalHelixLength(2),
      energyPenaltyHtype(9.0),
      energyPenaltyKtype(12.0),
#endif
      window_size(0),
      window_increment(0),
      delta(0),
    ...
    void help(char **argv)
    {
      std::cout << argv[0] << " ("
        ...
        << "   (If -e is set, -c will be ignored.)" << std::endl
        << "" << std::endl
        << "-z <int-value> Set minimal pseudoknot helix length" << std::endl
        << "   The first heuristic step in computung pseudoknots, is to find stable, non-" << std::endl
      ...
      while ((o = getopt(argc, argv, ":f:"
        ...
        switch (o) {
          ...
          case 's' :
        	  ...
        	  break;
          case 'x' :
        	  energyPenaltyHtype = std::atof(optarg);
        	  break;
          case 'y' :
        	  energyPenaltyKtype = std::atof(optarg);
        	  break;
#endif
          case 'k' :
            k = std::atoi(optarg);
